...
flare <- 
  read_delim("https://archive.ics.uci.edu/ml/machine-learning-databases/solar-flare/flare.data1", 
             ...
             col_names = c("ZClass", "Size", "Distr",
                          ...
                          "X_Class"),
             # col_types = "ffffffffffnnn"  Used before new lines below
             # am saving in case I want to revert back to this
             col_types = cols(ZClass = col_factor(levels = NULL),
                              Size = col_factor(levels = NULL),
                              Distr = col_factor(levels = NULL),
                              Activity = col_factor(levels = NULL), 
                              Evolution = col_factor(levels = NULL), 
                              Previous = col_factor(levels = NULL),
                              Complex = col_factor(levels = NULL), 
                              New = col_factor(levels = NULL), 
                              Area = col_factor(levels = NULL),
                              Largest = col_factor(levels = NULL), 
                              C_Class = col_integer(), 
                              M_Class = col_integer(),
                              X_Class = col_integer())
               )
...
The additive assumption is that $\partial Y/\partial X_i$ is independent from all $X_j$ where $i \neq j$.

Some models should not have the additive assumption.  Consider a predator-prey model where the change in the population of prey depends not just on the number of prey or the number of predators, but also depends on the *interaction* between the two.  
